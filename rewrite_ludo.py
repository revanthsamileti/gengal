...
# Header is lines 0 to 52 (which is up to `interface Props { ... }`)
header_lines = lines[:52]
header = "".join(header_lines)

# Logic is from line 1227 to the end
logic_start = 0
for i, line in enumerate(lines):
    if "export default function LudoBoardScreen" in line:
        logic_start = i
        break

logic_lines = lines[logic_start:]
logic_full = "".join(logic_lines)

# Now extract just the logic before the `return (` inside LudoBoardScreen
# We find the *last* `return (` or the `return (` that corresponds to the main component render.
# Usually it's `  return (`
logic = ""
import re
logic_match = re.search(r'(export default function LudoBoardScreen.*?)\n  return \(', logic_full, re.DOTALL)
if logic_match:
...
const BOARD_SIZE = Math.floor(Math.min(Dimensions.get('window').width, 400) * 0.9);
...
const wmStyles = StyleSheet.create({
  overlay: { ...StyleSheet.absoluteFillObject, backgroundColor: 'rgba(10,26,10,0.88)', alignItems: 'center', justifyContent: 'center', padding: 24, zIndex: 100 },
